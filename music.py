...
def play_first_chart_track():
    ...
    if len(chart.tracks) > 0:
        first_track = chart.tracks[0].track

        filename = './first_chart_track_YMusic.mp3'

        if not os.path.exists(filename):
            first_track.download(filename, codec='mp3', bitrate_in_kbps=128)
...
def play_by_track_id():
    ...
    if len(tracks) > 0:
        ...
        if not os.path.exists(filename):
            track.download(filename, codec='mp3', bitrate_in_kbps=128)
            exit_status = os.system(f'rhythmbox-client --play-uri {filename}')
            print(f'"rhythmbox-client --play-uri" command exit status: {exit_status}')
